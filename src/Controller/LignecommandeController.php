...
/**
    ...
    function mailing(Mailer $mailer){
        ...
        $mailer->mailing('khouloudbenmbarek2018@gmail.com');
        $this->addFlash("success", "commande effectuée avec succès! Merci de consulter votre mail");
        return  $this->redirectToRoute('AfficheLCF');
    }
 
    /**
     * @Route("/ImprimerLC/{id}", name="ImprimerLC")
     */
    ...
    /**
     * @Route("/stat2", name="blogdatalkl")
     */
    public function stat2(): Response
    {
        $user=$this->getUser();
        $lignecommande=$this->getDoctrine()->getManager()->getRepository(Lignecommande::class)->findAll();
        return $this->render('stat2.html.twig', [
            'controller_name' => 'BackaccController','us'=>$user, 'lignecommande'=>$lignecommande
        ]);
    }
